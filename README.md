...
- The human is represented as an ordinary Agent named `operator`, with no special powers, so
  ...
  stop a Team that crew can prove it started, peek at a pane, archive or restore an Agent,
  and run `prune`/`clean`. Every destructive one of those takes a single click in a dialog
  that names the irreversible effect — there is no phrase to type — and the request it then
  sends carries a `confirm: true` flag the server verifies for itself, so a bare POST can
  never fire one.
