...
/** The bold `**Term**:` definitions under CONTEXT.md's `## Language` section. */
...
describe('docs-site glossary mirrors CONTEXT.md', () => {
  ...
  it('teaches every term CONTEXT.md defines', () => {
    const rendered = new Set(siteTerms());
    const missing = contextTerms().filter((term) => !rendered.has(term));
    expect(
      missing,
