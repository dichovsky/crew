...
| [0012](./0012-optional-local-ui-server.md) | accepted | optional `crew ui` Console: started by the Operator, foreground-only, reachable only from the local machine, built on existing Store reads and actions; its presentation and confirmation specifics are superseded by ADR-0013 and its closed list of approved Operator actions is superseded by FR-U19 via ADR-0017 |
