...
`0.1.1` is the current published release on npm, and the first one published through the
OIDC pipeline, so it carries build provenance. The first release, `0.1.0`, could not:
because npm's OIDC Trusted Publishing can only attach to a package that already exists,
that first publish was a one-time manual `npm publish` (so `0.1.0` carries no CI
provenance). From `0.1.1` onward, publishing a GitHub Release runs
