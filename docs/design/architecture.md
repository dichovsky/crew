...
src/
  ...
  ui/
    server.ts                   token-guarded loopback HTTP/SSE server + static assets; routes GET /api/snapshot,/api/events,/api/health,/api/peek,/api/sessions,/api/resumable-sessions and the action POSTs
