...
- **FR-U27 — Stop through tmux adapter, and nothing else (ADR-0018).** An owned-Team stop shall
  ...
  `git worktree remove`. *Verify: automated test —
  `tests/integration/commands/team-stop.test.ts` (including the no-Worktree-removal pin, which
  asserts both subprocess seams and the tree's survival),
  `tests/integration/commands/teams.test.ts`, `tests/unit/format.test.ts`.*
