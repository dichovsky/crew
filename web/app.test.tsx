 ...
 * the mount fetch + render of the Overview, sidebar navigation across the five
 * views, the SSE-driven refetch (no polling), close() on unmount, the honest
 * error state and recovery, the message send (POST then refetch), the
 * "Message <agent>" pre-addressing, and the one-click destructive confirm.
 */
...
describe('App shell', () => {
  it('fetches on mount and renders the Overview with the crew roster', async () => {
    stubFetch();
  ...
  it('navigates between the five views from the sidebar', async () => {
    stubFetch();
    mocks.fetchSnapshot.mockResolvedValue(snapshotOf([task('t-1', 'Ship the widget')]));
    const host = mount();
...
describe('App actions', () => {
  it('pre-addresses the compose when a roster row is clicked and sends a message', async () => {
    const posts = stubFetch();
    mocks.fetchSnapshot.mockResolvedValue(snapshotOf([]));
    const host = mount();
    await vi.waitFor(() => expect(host.textContent).toContain('manager-1'));

    // Click the roster row → jumps to Messages pre-addressed to manager-1.
    (host.querySelector('.roster-row') as HTMLButtonElement).click();
    await vi.waitFor(() => expect(host.querySelector('.compose')).not.toBeNull());
    const select = host.querySelector('#compose-recipient') as HTMLSelectElement;
    expect(select.value).toBe('manager-1');

    const textarea = host.querySelector('#compose-body') as HTMLTextAreaElement;
    textarea.value = 'ping';
    textarea.dispatchEvent(new Event('input'));
    // Let the controlled-input state commit before the send reads it.
    await new Promise((resolve) => setTimeout(resolve, 0));
    const send = [...host.querySelectorAll('button')].find((b) =>
    ...
    // A refetch follows the completed POST.
    await vi.waitFor(() => expect(mocks.fetchSnapshot.mock.calls.length).toBeGreaterThan(1));
    unmount(host);
  });
